...
public interface PropertyInformationRepository extends JpaRepository<PropertyInformation, Integer> {
        ...
        @Query(value = "SELECT concat_ws(',', property_id, concat_ws(' ', unit_number, house_number, street_name, suburb_name, post_code)) FROM property_information WHERE tsv @@ plainto_tsquery('simple', ?1) LIMIT 1", nativeQuery = true)
